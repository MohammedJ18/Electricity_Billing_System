...
public class PayBill extends JFrame implements ActionListener {
    ...
    PayBill(String meter) {
        ...
        try (Connection con = Conn.connect();
            ...
            rs = pr.executeQuery("select * from bill where meter = '" + meter + "' AND month = 'January' ");
            while (rs.next()) {
        ...
        c1.addItemListener(new ItemListener() {
            ...
            public void itemStateChanged(ItemEvent ae) {
                try (Connection con = Conn.connect();
                    ...
                    ResultSet rs = pr.executeQuery("select * from bill where meter = '" + meter + "' AND month = '" + c1.getSelectedItem() + "'");
                    while (rs.next()) {
                        l13.setText(rs.getString("units"));
                        l14.setText(rs.getString("total_bill"));
                        l15.setText(rs.getString("status"));
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == b1) {
            try (Connection con = Conn.connect();
                ...
                pr.execute("update bill set status = 'Paid' where meter = '" + meter + "' AND month = '" + c1.getSelectedItem() + "'");
